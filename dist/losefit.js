...
exports.LoseFitStore = void 0;
exports.defaultLoseFitLogPath = defaultLoseFitLogPath;
exports.isLoseFitLogCommand = isLoseFitLogCommand;
exports.isLoseFitSummaryCommand = isLoseFitSummaryCommand;
exports.createEntry = createEntry;
exports.formatLoggedEntry = formatLoggedEntry;
exports.formatSummary = formatSummary;
const promises_1 = __importDefault(require("node:fs/promises"));
const node_path_1 = __importDefault(require("node:path"));
const zod_1 = require("zod");
const explicitCaloriesSchema = zod_1.z.object({ calories: zod_1.z.number().int().min(0).max(10000) });
const COMMON_ESTIMATES = [
...
function defaultLoseFitLogPath() {
    return node_path_1.default.resolve(process.cwd(), process.env.LOSEFIT_LOG_PATH || "../../data/losefit/meal-log.jsonl");
}
class LoseFitStore {
    logPath;
    constructor(logPath = defaultLoseFitLogPath()) {
        this.logPath = logPath;
    }
    get path() {
        return this.logPath;
    }
    async log(rawText, source = "tool", now = new Date()) {
        ...
        await promises_1.default.appendFile(this.logPath, `${JSON.stringify(entry)}\n`, "utf8");
        return entry;
    }
    async list() {
        try {
            const raw = await promises_1.default.readFile(this.logPath, "utf8");
            return raw
                .split(/\n+/)
                .map((line) => line.trim())
                .filter(Boolean)
    ...
    async summaryForLocalDate(date = localDate(new Date())) {
    ...
    async exportCsv(csvPath = this.logPath.replace(/\.jsonl$/i, ".csv")) {
        const entries = await this.list();
        await promises_1.default.mkdir(node_path_1.default.dirname(csvPath), { recursive: true });
        const rows = [
        ...
        await promises_1.default.writeFile(csvPath, rows.map((row) => row.map(csvEscape).join(",")).join("\n") + "\n", "utf8");
        return csvPath;
    }
}
exports.LoseFitStore = LoseFitStore;
function isLoseFitLogCommand(text) {
...
function isLoseFitSummaryCommand(text) {
...
function createEntry(rawText, source, now = new Date()) {
...
function formatLoggedEntry(entry) {
...
function formatSummary(summary) {
    if (!summary.entries.length)
        return `No LoseFit meals logged for ${summary.date} yet.`;
...
function extractMacro(text, label) {
...
function detectMealType(text, now) {
    const lower = text.toLowerCase();
    if (/breakfast/.test(lower))
        return "breakfast";
    if (/lunch/.test(lower))
        return "lunch";
    if (/dinner/.test(lower))
        return "dinner";
    if (/snack/.test(lower))
        return "snack";
...
function optionalSum(values) {
    const known = values.filter((item) => typeof item === "number");
