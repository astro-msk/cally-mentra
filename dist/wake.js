...
exports.looksLikeWake = looksLikeWake;
exports.extractWakeCommand = extractWakeCommand;
exports.parseExtraVariants = parseExtraVariants;
...
function normalizePhonetic(token) {
...
function looksLikeWake(token, opts = {}) {
    const bare = token.toLowerCase().replace(/[^a-z]/g, "");
    if (!bare)
        return false;
    if (EXPLICIT_VARIANTS.has(bare))
        return true;
    if (opts.extraVariants?.includes(bare))
        return true;
    if (STOPWORDS.has(bare))
        return false;
    if (opts.fuzzy === false)
        return false;
    const norm = normalizePhonetic(bare);
    if (norm.length < 3 || !norm.startsWith("c"))
        return false;
 ...
 */
function extractWakeCommand(text, opts = {}) {
...
function parseExtraVariants(raw) {
    if (!raw)
        return [];
    return raw
        .split(",")
